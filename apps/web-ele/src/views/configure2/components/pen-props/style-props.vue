    ...
    <t-form label-align="left" v-if="pen">
      ...
      <t-form-item label="线条" name="dash">
        <t-select v-model="pen.dash" @change="changeValue('dash')">
          <t-option :key="0" :value="0" label="实线" />
          <t-option :key="1" :value="1" label="虚线" />
        </t-select>
      </t-form-item>
      ...
      <t-form-item label="文字水平对齐" name="textAlign">
        <t-select v-model="pen.textAlign" @change="changeValue('textAlign')">
          <t-option key="left" value="left" label="左对齐" />
          <t-option key="center" value="center" label="居中" />
          <t-option key="right" value="right" label="右对齐" />
        </t-select>
      </t-form-item>
      <t-form-item label="文字垂直对齐" name="textBaseline">
        ...
        >
          <t-option key="top" value="top" label="顶部对齐" />
          <t-option key="middle" value="middle" label="居中" />
          <t-option key="bottom" value="bottom" label="底部对齐" />
        </t-select>
      </t-form-item>
...
.props-panel :deep(.t-form) .t-space {
  gap: 4px;
}
</style>
