...
export class Meta2d {
  ...
  setTheme(theme: string) {
    ...
    // 更新全局的主题css变量
    if (
      !(this.store.options.themeOnlyCanvas || this.store.data.themeOnlyCanvas)
    ) {
        ...
        if (url.includes('${')) {
          ...
          if (keys) {
            keys?.forEach((key) => {
              url = url.replace(
                `\${${key}}`,
                pen[key] || this.getDynamicParam(key),
              );
            });
          }
        }
            ...
            if (keys) {
              keys.forEach((key) => {
                _value = _value.replace(
                  `\${${key}}`,
                  getter(cpen, key) || this.getDynamicParam(key),
                );
              });
            }
  ...
  convertType(value: string, type: string) {
    if (typeof value === 'string') {
      if (['switch', 'bool', 'boolean'].includes(type)) {
        ...
        ['integer', 'number', 'int', 'enum', 'double', 'float'].includes(type)
      ) {
  ...
  loadLineAnimateDraws() {
    ...
    Object.entries(this.store.data.lineAnimateDraws).forEach(
      ([key, drawFunc]) => {
        // @ts-ignore
        globalStore.lineAnimateDraws[key] = new Function(
          'ctx',
          'pen',
          'state',
          'index',
  ...
  statistics() {
    ...
    const imgDrawNum = this.store.data.pens.filter(
      (pen) => pen.image && pen.calculative.inView,
    ).length;
    ...
    this.store.data.pens.forEach((pen) => {
        ...
        if (realTime.bind && realTime.bind.id) {
          ...
          if (realTime.bind.class === 'iot') {
            ...
            let index = properties.findIndex(
              (item) => item.key === realTime.bind.id,
            );
            if (index === -1) {
              properties.push({
            ...
            let bind = realTime.bind.id.split('#');
            const sql = this.store.data.sqls.find(
              (item) => item.bindId === bind[0],
            );
            if (sql) {
              if (!sql.keys) {
                sql.keys = [];
      ...
      pen.events?.forEach((event) => {
        const actions = event.actions?.filter(
          (item) => item.action === EventAction.SendData,
        );
  ...
  registerLineAnimateDraws = (name, drawFunc) => {
    ...
    // 同步到store
    // @ts-ignore
    globalStore.lineAnimateDraws[name] = new Function(
      'ctx',
      'pen',
      'state',
      'index',
  ...
  updateRectbyChild(rect: Rect, child: Pen, parent: Pen) {
    ...
    if (
      parent.container &&
      rectInRect(rect, parent.calculative.worldRect, true)
    ) {
      //取所有图元的范围
  ...
  async connectIot() {
    ...
    const token = await this.getIotToken(
      iot.devices,
      iot.protocol === 'websocket' ? 1 : undefined,
    );
    iot.token = token;
    //物联网设备
    ...
    this.iotTimer = setInterval(() => {
      this.iotMqttClient &&
        this.iotMqttClient.publish(`le5le-iot/subscribe/ping`, token);
    }, 300000);
  ...
  connectSSE(net: Network) {
    ...
    this.eventSources[net.index] = new EventSource(net.url, {
      ...
      if (keys) {
        keys.forEach((key) => {
          options.username = options.username.replace(
            `\${${key}}`,
            this.getDynamicParam(key),
          );
        });
      }
    }
    if (options?.password && options.password.includes('${')) {
      let keys = options.password
        .match(/\$\{([^}]+)\}/g)
        ?.map((m) => m.slice(2, -1));
      if (keys) {
        keys.forEach((key) => {
    ...
    this.mqttClients[net.index].on('connect', (connack) => {
      ...
      if (!connack.sessionPresent) {
        ...
        if (net.topics) {
          ...
          this.mqttClients[net.index].subscribe(
            topics.split(','),
            { qos: 1 },
            (err) => {
    ...
    if (method === 'list') {
      ...
      if (sql.dbType === 'oracle') {
        ...
        if (!_sql.includes('LIMIT')) {
          _sql +=
            ` LIMIT ${sql.pageSize || 20}` +
            (sql.current > 1
              ? ' OFFSET ' + (sql.current - 1) * (sql.pageSize || 20)
              : '');
        }
      }
    }
    ...
    if (res.ok) {
      ...
      if (data) {
        ...
        sql.keys?.forEach((key) => {
          arr.push({
            id: sql.bindId + '#' + key,
            value: getter(data, key.split('#').join('.')),
          });
        });

        arr.push({ id: sql.bindId, value: data });
        this.socketCallback(JSON.stringify(arr), {
          type: 'sql',
          url: `/api/iot/data/sql/${method}`,
          method,
        });
      }
    }
  ...
  getDynamicParam(key: string) {
    ...
    if (globalThis.le5leTokenD) {
      let tokenkeys = [
        globalThis.le5leSSOTokenName ?? 'ssotoken',
        globalThis.le5leTokenName ?? 'token',
      ];
    ...
    let params = queryURLParams();
    let value =
      params[key] || lsValue || getCookie(key) || globalThis[key] || '';
    return value;
  }
  ...
  async requestHttp(_req: Network) {
    ...
    if (req.url) {
      ...
      if (res.ok) {
        const data = await res.text();
        const net = this.store.data.networks.filter(
  ...
  socketCallback(
    ...
    if (context.net?.socketCbJs) {
      if (!context.net?.socketFn) {
        context.net.socketFn = new Function(
          'e',
          'context',
          context.net.socketCbJs,
        ) as (
          e: string,
          context?: {
            meta2d?: Meta2d;
  ...
  setValue(
    ...
    pens.forEach((pen) => {
      if (pen.name === 'echarts' && !pen.onBeforeValue) {
        const hasEchartsStartKey = Object.keys(data).some((key) =>
          echartReg.test(key),
        );
        ...
        if (e.pen && e.pen.formId) {
          ...
          if (e.pen.formType === 'submit') {
            this.store.data.locked &&
              formPen &&
              !formPen.disabled &&
              this.doEvent(formPen, 'submit');
          } else if (e.pen.formType === 'reset') {
            reset(e.pen);
            this.store.data.locked &&
              formPen &&
              !formPen.disabled &&
              this.doEvent(formPen, 'reset');
          }
        }
        e.pen &&
          e.pen.onClick &&
