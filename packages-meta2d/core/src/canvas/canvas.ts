...
export class Canvas {
  ...
  mind = mind;
  mouseDown: { restore?: boolean; x: number; y: number };
  ...
  constructor(
    ...
    // https://doc.le5le.com/document/119591668?id=119591668
    ...
    parentElement.append(this.externalElements);
    this.createInput();

    this.tooltip = new Tooltip(parentElement, store);
    this.tooltip.box.addEventListener('mouseleave', (e) => {
      this.patchFlags = true;
      this.store.lastHover && (this.store.lastHover.calculative.hover = false);
      const hover = this.store.data.pens.find(
  ...
  active(pens: Pen[], emit = true) {
    if (this.store.active && this.store.active.length > 0) {
  ...
  activeHistory() {
    const now = this.store.histories[this.store.historyIndex + 1];
    const pens = [];
    ...
    const before = this.store.histories[this.store.historyIndex];
    if (
      before &&
      (before.type === EditType.Add || before.type === EditType.Delete)
    ) {
  ...
  alignPenToGrid(pen: Pen) {
    ...
    if (autoAlignGrid && !pen.type) {
      ...
      // 算出偏移了多少个网格
      const m = Number.parseInt((rect.x / gridSize).toFixed(0));
      const n = Number.parseInt((rect.y / gridSize).toFixed(0));
      const x1 = m * gridSize;
      const y1 = n * gridSize;
      // 算出最终的偏移坐标
  ...
  calcuActiveFit() {
    ...
    const downX =
      (this.mouseDown.x - this.store.data.origin.x) /
      this.store.data.scale /
      width;
    const downY =
      (this.mouseDown.y - this.store.data.origin.y) /
      this.store.data.scale /
      height;
  ...
  convertSpecialCharacter(str) {
    ...
    return str.replaceAll(/&(lt|gt|nbsp|amp|quot);/gi, (all, t) => {
  ...
  async copy(pens?: Pen[], emit = true) {
    ...
    const copyPens: Pen[] = this.getAllByPens(
  ...
  destroy() {
    ...
    this.externalElements.addEventListener('dragover', (e) =>
      e.preventDefault(),
    );
  ...
  doEditAction(action: EditAction, undo: boolean) {
    ...
    switch (action.type) {
      ...
      case EditType.Delete: {
        action.pens.reverse().forEach((aPen) => {
          ...
          if (pen.path) {
            !this.store.data.paths[pen.pathId] &&
              (this.store.data.paths[pen.pathId] = pen.path);
          }
          // 先放进去，pens 可能是子节点在前，而父节点在后
          this.store.pens[pen.id] = pen;
          if (pen.type && pen.lastConnected) {
            for (const key in pen.lastConnected) {
              this.store.pens[key] &&
        ...
        pens.forEach((p) => {
          ...
          if (i !== -1) {
            ...
            if (
              ...
              for (const key in this.store.data.pens[i].lastConnected) {
                if (this.store.pens[key]) {
                  const connected = deepClone(
            ...
            if (pen.calculative.canvas.parent.isCombine(pen)) {
              const unPen: Pen = unPens.find((item) => item.id === pen.id);
    ...
    if (action.type === EditType.Update) {
      const pens = [...action.pens, ...action.initPens];
    ...
    if (Object.keys(this.randomIdObj).length > 0) {
      ...
      for (const pen of pens) {
        ...
        if (pen.animations?.length) {
          const str = JSON.stringify(pen.animations).replace(
            regex,
            (match) => this.randomIdObj[match],
          );
          pen.animations = JSON.parse(str);
        }
        if (pen.triggers?.length) {
          const str = JSON.stringify(pen.triggers).replace(
            regex,
            (match) => this.randomIdObj[match],
          );
        ...
        if (pen.events?.length) {
          const str = JSON.stringify(pen.events).replace(
            regex,
            (match) => this.randomIdObj[match],
          );
    ...
    for (const pen of pens) {
      if (!pen.parentId) {
        ...
        if ((pen as any).dataset) {
          lastW = num % 2 === 0 ? pen.width - 40 * this.store.data.scale : 0;
          num++;
          if (num % 2 === 0) {
  ...
  find(idOrTag: string) {
    return this.store.data.pens.filter((pen) => {
      return pen.id == idOrTag || (pen.tags && pen.tags.includes(idOrTag));
    });
  }
  ...
  async finishDrawline(end?: boolean) {
    ...
    if (!from.connectTo || !to.connectTo) {
      if (this.store.options.disableEmptyLine) {
        // 有一端未连线，且 禁止创建空线条
        if (from.connectTo) {
          this.store.pens[from.connectTo].connectedLines = this.store.pens[
            from.connectTo
          ].connectedLines.filter(
            (item) => item.lineId !== this.drawingLine.id,
          );
        }
        this.drawingLine = undefined;
        this.render();
    ...
    pen.frames &&
      pen.frames.forEach((frame) => {
        for (const key in frame) {
          if (
            !['duration', 'height', 'rotate', 'width', 'x', 'y'].includes(
              key,
            ) &&
            !initProps[key]
          ) {
  ...
  getSpecialAngle(to: Point, last: Point) {
    // 快捷定位到特殊角度
    let angle = 0;
    const angleArr = [0, 30, 45, 60, 90, 120, 150, 135, 180];
    // 获取实际角度
    if (to.x - last.x === 0) {
      if (last.y > to.y) {
        angle = 90;
  ...
  gotoView(x: number, y: number) {
    ...
    if (!isFinite(rect.width)) {
      throw new TypeError('can not move view, because width is not finite');
    }
    const width = this.store.data.width || this.store.options.width;
    const height = this.store.data.height || this.store.options.height;
    if (width && height) {
  ...
  hideInput = () => {
    ...
    if (this.inputParent.style.display === 'flex') {
      ...
      if (pen.onInput) {
        ...
        pen.calculative.textLines.length === 0
      ) {
        calcTextRect(pen);
      }
  ...
  inAnchor(pt: Point, pen: Pen, anchor: Point): HoverType {
    ...
    if (
      ...
      if (pen.type) {
        ...
        this.externalElements.style.cursor =
  ...
  listen() {
    ...
    this.externalElements.addEventListener('drop', this.ondrop);
    this.externalElements.addEventListener('contextmenu', (e) =>
      e.preventDefault(),
    );
    this.store.options.interval = 50;
    this.externalElements.addEventListener('touchstart', this.ontouchstart);
    this.externalElements.addEventListener('touchmove', this.ontouchmove);
    this.externalElements.addEventListener('touchend', this.ontouchend);
    this.externalElements.addEventListener('mousedown', (e) => {
      this.onMouseDown({
        x: e.offsetX,
        y: e.offsetY,
        clientX: e.clientX,
    ...
    this.externalElements.addEventListener('mousemove', (e) => {
    ...
    this.externalElements.addEventListener('mouseup', (e) => {
      this.onMouseUp({
        x: e.offsetX,
        y: e.offsetY,
        clientX: e.clientX,
    ...
    this.externalElements.addEventListener('mouseleave', (e) => {
      // 离开画布取消所有选中
    ...
    this.externalElements.addEventListener('dblclick', this.ondblclick);
    this.externalElements.tabIndex = 0;
    this.externalElements.addEventListener('blur', () => {
      this.mouseDown = undefined;
    });
    this.externalElements.addEventListener('wheel', this.onwheel);
  ...
  makeFit() {
    ...
    const width = this.store.data.width || this.store.options.width;
    const height = this.store.data.height || this.store.options.height;
    const x = (Math.floor(_rect.x) - this.store.data.origin.x) / scale / width;
    const y = (Math.floor(_rect.y) - this.store.data.origin.y) / scale / height;
    const rect: Fit = {
      x,
      y,
  ...
  makePen(pen: Pen) {
    ...
    if (pen.image && pen.name !== 'gif' && pen.canvasLayer === undefined) {
      pen.canvasLayer = pen.isBottom
        ? CanvasLayer.CanvasImageBottom
    ...
    if (pen.video || pen.audio) {
      pen.calculative.addEventListener('ended', (pen: Pen) => {
        this.nextAnimate(pen);
      });
    }
    for (const k in pen) {
  ...
  moveLineAnchor(
    ...
    keyOptions: { altKey?: boolean; ctrlKey?: boolean; shiftKey?: boolean },
  ) {
    ...
    const anchorId = this.store.activeAnchor?.id;
    const connectedLine = this.store.pens[
    ...
    if (line.lineName === 'polyline' && !keyOptions.shiftKey) {
      ...
      if (line.lineName === 'line') {
        ...
        const relativePt = line.calculative.worldAnchors[index - 1];
        if (keyOptions.ctrlKey && keyOptions.shiftKey) {
          const _pt = deepClone(pt);
    ...
    if (pen.nextAnimate) {
      pens = this.store.data.pens.filter((p) => {
        return (
          ...
          (p.tags && p.tags.includes(pen.nextAnimate))
        );
      });
    }
    ...
    pens.forEach((pen) => {
      if (pen.calculative.pause) {
        ...
        if (pen.name === 'video') {
          ...
          pen.type ||
          pen.frames?.length ||
          (pen.animations && pen.animations.length > 0)
        ) {
          // 存储动画初始状态
          if (pen.type) {
            if (pen.animations?.length) {
            ...
            if (!pen.frames && pen.animations && pen.animations.length > 0) {
              // 无活动动画
              const autoIndex = pen.animations?.findIndex((i) => i.autoPlay);
              const index = autoIndex === -1 ? 0 : autoIndex;
  ...
  ondrop = async (event: DragEvent) => {
    ...
    if (!obj) {
      ...
      if (
        ...
        !(this.addCaches && this.addCaches.length > 0)
      ) {
        // 必须是图片类型
        const isGif = files[0].type === 'image/gif';
        obj = await this.fileToPen(files[0], isGif);
      } else if (this.addCaches && this.addCaches.length > 0) {
        obj = this.addCaches;
        this.addCaches = [];
      } else {
    ...
    switch (e.key) {
      ...
      case 'Enter': {
        if (this.drawingLineName) {
          ...
          this.drawingLineName = this.store.active[0].anchors[0].connectTo
            ? ''
            : this.store.options.drawingLineName;
        }
        ...
        case HoverType.Node: {
          if (this.store.hover) {
            if (
              ...
              this.store.active.length === 1 &&
              this.store.hover.id === this.store.active[0].id
            ) {
              // 准备移动子图元
              this.calcActiveRect();
              break;
            ...
            if (
              parentPen &&
              (parentPen.container ||
                this.store.options.containerShapes?.includes(parentPen.name))
            ) {
            ...
            if (e.ctrlKey && !e.shiftKey) {
              ...
              if (
                (!(
                  this.activeRect &&
                  pointInRect({ x: e.x, y: e.y }, this.activeRect)
                ) ||
                  this.store.active.length == 1) &&
                !pen.calculative.active
              ) {
  ...
  onMouseMove = (e: {
    ...
    if (
      ...
      !this.store.data.disableTranslate
    ) {
      // 画布平移前提
      ...
      if (!this.drawingLine && !this.pencil) {
        ...
        if (
          e.buttons === 1 &&
          (e.ctrlKey || (!this.hoverType && !this.hotkeyType)) &&
          !(e.ctrlKey && (this.store.activeAnchor || this.store.active?.length))
        ) {
          this.dragRect = {
        ...
        if (this.movingAnchor) {
          ...
          if (this.hoverType === HoverType.LineAnchor) {
            if (
              (this.dockInAnchor(e) ||
                this.store.active[0]?.lineName === 'line') &&
  ...
  onMouseUp = (e: {
    ...
    // Add pen
    if (this.addCaches && this.addCaches.length > 0) {
      if (!this.store.data.locked) {
        if (
          this.dragRect && // 只存在一个缓存图元
    ...
    if (this.willInactivePen) {
      ...
      const index = this.store.active.indexOf(this.willInactivePen);
      if (index !== -1) {
    ...
    if (navigator.clipboard && event.clipboardData) {
      ...
      if (items) {
        for (const item of items) {
          if (item.type.includes('image') && item.getAsFile()) {
            hasImages = true;
            break;
          }
        }
      ...
      if (items) {
        for (const item of items) {
          if (item.type.includes('image') && item.getAsFile()) {
            ...
            const name = item.type.slice(6) === 'gif' ? 'gif' : 'image';
            if (blob !== null) {
  ...
  onwheel = (e: WheelEvent) => {
    ...
    if (
      Math.abs((e as any).wheelDelta) > 100 && // 鼠标滚轮滚动 scroll模式下是上下滚动而不是缩放 ctrl可以控制缩放
      this.store.options.scroll &&
      this.scroll &&
      !this.store.options.scrollButScale &&
      !(e.ctrlKey || e.metaKey)
    ) {
      this.scroll.wheel(e.deltaY < 0);
      return;
    ...
    if (this.store.options.scaleOff) {
      ...
      if (isMac) {
        ...
        let offset = 0.2;
        if (e.deltaY.toString().includes('.')) {
          offset = 0.01;
        }
        scaleOff = e.deltaY > 0 ? -offset : offset;
      }
    }
  ...
  async paste() {
    ...
    if (clipboardText) {
      try {
        ...
        console.warn('剪切板数据不是json', error.message);
        return;
      }
    ...
    const scale = this.store.data.scale;
    if (
      this.store.clipboard.mousePos &&
      (Math.abs(this.store.clipboard.mousePos.x - this.mousePos.x) >
        100 * scale ||
        Math.abs(this.store.clipboard.mousePos.y - this.mousePos.y) >
          100 * scale)
    ) {
      const _x =
        -this.store.clipboard.initRect.width /
        this.store.clipboard.scale /
        10 /
        scale;
      const _y =
        -this.store.clipboard.initRect.height /
        this.store.clipboard.scale /
        10 /
        scale;
      let offsetX =
      ...
      if (scale < this.store.clipboard.scale) {
        ...
        offsetX =
          (((scale - this.store.clipboard.scale) /
            ((this.store.clipboard.scale - scale) * 100)) *
            this.store.clipboard.initRect.width) /
            2 +
          _x;
        offsetY =
      ...
      if (this.store.clipboard.pens.length > 1) {
        offsetX =
          ((scale - 1) * this.store.clipboard.initRect.width) /
          this.store.clipboard.scale /
          2;
        offsetY =
          ((scale - 1) * this.store.clipboard.initRect.height) /
          this.store.clipboard.scale /
          2;
      }
  ...
  pensToPng(
    ...
    const ids = new Set(allPens.map((pen) => pen.id));
    const rect = getRect(allPens);
    if (!isFinite(rect.width)) {
      throw new TypeError('can not to png, because width is not finite');
    }
    ...
    if (
      ...
      (!navigator.userAgent.includes('Firefox') &&
        canvas.height * canvas.width > 268_435_456) ||
      (navigator.userAgent.includes('Firefox') &&
        canvas.height * canvas.width > 472_907_776)
    ) {
      throw new Error(
    ...
    const background = this.store.options.downloadBgTransparent
      ? undefined
      : this.store.data.background || this.store.styles.background;
  ...
  randomCombineId(pen: Pen, pens: Pen[], parentId?: string) {
    ...
    if (pen.type) {
      beforeIds =
        pen.anchors[0].connectTo ||
        pen.anchors[pen.anchors.length - 1].connectTo
          ? [pen.id, pen.anchors[0].id, pen.anchors[pen.anchors.length - 1].id]
          : [pen.id];
    } else {
      if (pens.length > 1) {
  ...
  renderBorder = () => {
    if (
      !this.store.data.locked && // Occupied territory.
      this.activeRect &&
      ...
      ctx.globalAlpha =
        this.store.options.activeGlobalAlpha === undefined
          ? 0.3
  ...
  setInputStyle = (pen: Pen) => {
    ...
    if (pen.textWidth) {
      ...
      if (pen.whiteSpace !== 'pre-line') {
        style +=
          _textWidth < pen.fontSize
            ? `width:${pen.fontSize * 1.2 * font_scale}px;`
            : `width:${
                scale > 1
    ...
    if (pen.whiteSpace !== 'nowrap') {
      const textWidth = pen.fontSize * 1.2 * pen.text.length;
      const contentWidth =
  ...
  showInput = (pen: Pen, rect?: Rect, background = 'transparent') => {
    ...
    // value和innerText问题
    const preInputText =
      pen.calculative.tempText === undefined
        ? `${pen.text}` || ''
        : pen.calculative.tempText;
    const textArr = preInputText.replaceAll(' ', '&nbsp;').split(/\s/);
    ...
    this.inputParent.style.left = `${textRect.x + this.store.data.x - (pen.calculative.textLeft || 0)}px`; // + 5
    this.inputParent.style.top = `${textRect.y + this.store.data.y - (pen.calculative.textTop || 0)}px`; // + 5
    const _width = textRect.width; // + (pen.textLeft || 0);
    ...
    this.inputParent.style.transform =
      pen.rotate % 360 ? `rotate(${pen.rotate}deg)` : null;
    ...
    if (pen.dropdownList && this.dropdown.style.display !== 'block') {
      ...
      // }
      this.dropdown.style.background =
        pen.dropdownBackground || this.store.styles.popContentBg || '#fff';
  ...
  splitLine(line: Pen, anchor: Point): void {
    ...
    const index = worldAnchors.indexOf(anchor);
  ...
  translatePens(
    ...
    const hasLocked = pens.some((item: Pen) => {
  ...
  updateFitRect(fit: Fit = this.canvasImage.activeFit) {
    ...
    if (fit.left) {
      if (fit.leftValue) {
        fit.x =
          Math.abs(fit.leftValue) < 1 ? fit.leftValue : fit.leftValue / width;
      } else {
        fit.x = 0;
      }
    }
    if (fit.right) {
      fit.width = fit.rightValue
        ? 1 -
          (Math.abs(fit.rightValue) < 1
            ? fit.rightValue
            : fit.rightValue / width) -
          fit.x
        : 1 - fit.x;
    }
    if (fit.top) {
      if (fit.topValue) {
        fit.y =
          Math.abs(fit.topValue) < 1 ? fit.topValue : fit.topValue / height;
      } else {
        fit.y = 0;
      }
    }
    if (fit.bottom) {
      fit.height = fit.bottomValue
        ? 1 -
          (Math.abs(fit.bottomValue) < 1
            ? fit.bottomValue
            : fit.bottomValue / height) -
          fit.y
        : 1 - fit.y;
    }
    this.canvasImage.init();
  ...
  updateValue(pen: Pen, data: IValue): void {
    ...
    for (const k in data) {
      ...
      if (k.includes('.')) {
        ...
        if (['borderRadius', 'close', 'lineSmooth', 'name'].includes(k)) {
          willUpdatePath = true;
        }
        ...
        if (
          pen.image &&
          pen.name !== 'gif' &&
          needImgCanvasPatchFlagsProps.includes(k)
        ) {
          willRenderImage = true;
        }
      }
  ...
  private addRuleLine(e: { ctrlKey?: boolean; x: number; y: number }) {
  ...
  private createInput() {
    ...
    this.inputParent.append(this.inputDiv);
    // this.inputParent.appendChild(this.inputRight);
    this.dropdown.addEventListener('mouseleave', () => {
      this.store.hover = null;
    });
    this.inputParent.append(this.dropdown);
    this.externalElements.append(this.inputParent);
    this.inputParent.addEventListener('mousedown', this.stopPropagation);
    this.inputDiv.addEventListener('mousedown', this.stopPropagation);
    this.inputDiv.contentEditable = 'false';
    // this.inputRight.onmousedown = this.stopPropagation;
    this.dropdown.addEventListener('mousedown', this.stopPropagation);
    ...
    this.inputDiv.addEventListener('focus', (e: any) => {
      if (navigator.userAgent.includes('Firefox')) {
        if (!e.target.innerText) {
          let left = this.inputDiv.offsetWidth / 2;
          const inputDivStyle = window.getComputedStyle(this.inputDiv, null);
        ...
        if (e.target.innerText) {
          ...
          if (inputDivStyle.justifyContent === 'center') {
            this.inputDiv.style.paddingTop = ` ${
              ...
              Number.parseFloat(inputDivStyle.lineHeight) / 2
            }px`;
          }
        }
      }
    ...
    this.inputDiv.addEventListener('input', (e: any) => {
      const pen = this.store.pens[this.inputDiv.dataset.penId];
      if (pen && pen.inputType === 'number') {
        const value = e.target.innerText;
        const numericValue = value.replaceAll(/\D/g, ''); // 移除非数字字符
        // 如果输入的值不是纯数字，则替换为纯数字
    ...
    this.inputDiv.addEventListener('keyup', (e: KeyboardEvent) => {
    ...
    this.inputDiv.addEventListener('mousedown', this.stopPropagation);
    this.inputDiv.addEventListener('wheel', (e) => {
      e.stopPropagation();
    });
  ...
  private dropdownAppendOption(text: string, index: number) {
    ...
    li.addEventListener('wheel', this.stopPropagation);
    li.innerText = text;
    li.style.overflow = 'hidden';
    li.style.textOverflow = 'ellipsis';
    li.title = text;
    (li.style as any).zoom = this.store.data.scale;
    li.addEventListener('mousedown', this.stopPropagation);
    li.dataset.i = `${index}`;
    li.addEventListener('click', this.selectDropdown);
    const pen = this.store.pens[this.inputDiv.dataset.penId];
    li.addEventListener('mouseenter', () => {
      li.style.background =
        pen.dropdownHoverBackground || this.store.styles.activeBg || '#eee';
  ...
  private firefoxLoadSvg(pen: Pen) {
    ...
    request.addEventListener('load', () => {
      ...
      const inlineSVG = result.querySelectorAll('svg')[0];
  ...
  private getContainerHover = (pt: Point) => {
    ...
    const containerPens: Pen[] = this.store.data.pens.filter(
      (pen) =>
        pen.container || this.store.options.containerShapes?.includes(pen.name),
    );
    if (containerPens.length > 0) {
        ...
        if (pointInRect(pt, pen.calculative.worldRect)) {
          ...
          if (pen === this.store.hoverContainer) {
            ...
            if (this.store.lastHoverContainer !== this.store.hoverContainer) {
              this.patchFlags = true;
              const movingPen =
                this.store.lastHoverContainer.calculative.canvas.store.pens[
                  this.store.lastHoverContainer.id + movingSuffix
                ];
    ...
    if (
      ...
      if (!activePensLock && !activePensDisableRotate) {
        ...
        if (this.activeRect.rotate) {
          rotatePoint(
            rotatePt,
            this.activeRect.rotate,
            this.activeRect.pivot || this.activeRect.center,
          );
        }
        // 旋转控制点
  ...
  private inFitBorder = (pt: Point) => {
    ...
    if (
      point.y > height * (fit.y + fit.height) - 10 &&
      point.y < height * (fit.y + fit.height) + 10
    ) {
      current = 'bottom';
      this.externalElements.style.cursor = 'row-resize';
    ...
    if (
      point.x > width * (fit.x + fit.width) - 10 &&
      point.x < width * (fit.x + fit.width) + 10
    ) {
      current = 'right';
      this.externalElements.style.cursor = 'col-resize';
      ...
      if (pen.type) {
        ...
        if (pos) {
          if (!this.store.data.locked && !pen.locked) {
            this.externalElements.style.cursor =
              this.hotkeyType === HotkeyType.AddAnchor ? 'pointer' : 'move';
          } else {
        ...
        isIn =
          pen.name === 'line'
            ? pointInSimpleRect(pt, pen.calculative.worldRect, pen.lineWidth)
            : pointInRect(pt, pen.calculative.worldRect);
        if (isIn) {
  ...
  private movedActivePens(readyConnect?: boolean) {
    // 鼠标松手才更新，此处是更新前的值
    // follower
    const movedPens: Pen[] = this.getAllFollowersByPens(
      this.store.active,
      false,
    );
    ...
    const { origin, scale } = this.store.data;
    const autoAlignGrid =
      this.store.options.autoAlignGrid &&
      (this.store.data.grid || this.store.options.grid);
    movedPens.forEach((pen) => {
      const i = this.movingPens.findIndex(
        (item) => item.id === pen.id + movingSuffix,
      );
      if (i === -1) {
        return;
      }
    ...
    if (
      ...
      if (this.store.hover.onShowInput) {
        ...
        if (this.store.hover && this.store.hover.parentId) {
          if (
            this.store.active?.length === 1 &&
            this.store.active[0].id === this.store.hover.id
          ) {
            this.showInput(this.store.hover);
          } else {
            this.store.pens[this.store.hover.parentId].children.forEach(
              (id) => {
                ...
                (_id) => {
                  ...
                  if (pointInRect(pt, pen.calculative.worldRect)) {
                    const dis = Math.hypot(
                      pt.x - pen.calculative.worldRect.center.x,
                      pt.y - pen.calculative.worldRect.center.y,
                    );
                    if (dis < distance) {
                      distance = dis;
                      id = _id;
  ...
  private pastePen = (pen: Pen, parentId?: string) => {
    ...
    if (!pen.parentId) {
      ...
      if (
        this.keyOptions &&
        this.keyOptions.altKey &&
        (this.keyOptions.ctrlKey || this.keyOptions.metaKey)
      ) {
        pen.x = -this.store.data.x + this.width / 2 - pen.width / 2;
        pen.y = -this.store.data.y + this.height / 2 - pen.height / 2;
      } else if (
        this.keyOptions &&
        this.keyOptions.shiftKey &&
        (this.keyOptions.ctrlKey ||
          this.keyOptions.metaKey ||
          this.keyOptions.F)
      ) {
      } else {
