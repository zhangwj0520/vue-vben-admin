...
export class Canvas {
  ...
  onMouseUp = (e: {
    ...
    if (this.willInactivePen) {
      ...
      const index = this.store.active.findIndex(
        (p) => p === this.willInactivePen,
      );
      if (index >= 0) {
            ...
            if (this.store.hover.parentId) {
              ...
              this.store.pens[this.store.hover.parentId]?.children?.forEach(
                (_id) => {
                  ...
                  if (pointInRect(pt, pen.calculative.worldRect)) {
                    const dis = Math.sqrt(
                      (pt.x - pen.calculative.worldRect.center.x) ** 2 +
                        (pt.y - pen.calculative.worldRect.center.y) ** 2,
                    );
                    if (dis < distance) {
                      distance = dis;
                      id = _id;
