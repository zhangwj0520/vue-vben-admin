  ...
  pen.calculative.textLines &&
    pen.calculative.textLines.forEach((text: string) => {
      ...
      if (pen.calculative.textType) {
        ...
        width =
          getFontWith(text, pen) + text.length * pen.calculative.letterSpacing;
      } else {
        width =
          ctx.measureText(text).width +
          text.length * pen.calculative.letterSpacing;
      }
...
export function calcTextAutoWidth(pen: Pen) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (canvas.store.options.measureTextWidth) {
      ...
      currentWidth =
        ctx.measureText(arr[i]).width +
        arr[i].length * pen.calculative.letterSpacing; //* scale;
    } else {
      // 近似计算
      ...
      currentWidth =
        ...
        spaceWidth +
        otherWidth +
        arr[i].length * pen.calculative.letterSpacing;
    }
