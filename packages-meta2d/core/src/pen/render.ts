...
function getImagePosition(pen: Pen) {
  ...
  if (!rect) {
    return {
      ...
      y: worldRect.y,
      width:
        worldRect.width || imgNaturalWidth || pen.calculative.img.naturalWidth,
      height:
        worldRect.width ||
        imgNaturalHeight ||
        pen.calculative.img.naturalHeight,
    };
  }
  ...
  switch (pen.lineAnimateType) {
    ...
    case LineAnimateType.Custom:
      switch (pen.lineAnimateTargetType) {
        case lineAnimateTargetType.Image:
          pen.lineAnimateImages ??= [];
          if (pen.lineAnimateImages.length > 0) {
            const animateImgs: any = pen.lineAnimateImages.map((src, index) => {
              const image = globalStore.htmlElements[src];
              if (!image) {
                pen.calculative.canvas.__loadImage(src);
              } else {
                return image;
              }
            });
            setElementAnimateOnLine(ctx, pen, 'image', animateImgs);
          }
          break;
        case lineAnimateTargetType.Icon:
          const icon = pen.lineAnimateIcon;
          setElementAnimateOnLine(ctx, pen, 'icon', icon);
          break;
        case lineAnimateTargetType.Pen:
          if (pen.lineAnimatePens) {
            setElementAnimateOnLine(ctx, pen, 'pen', pen.lineAnimatePens);
          }
          break;
        case lineAnimateTargetType.Element:
          const animateEle = pen.lineAnimateElement;
          setElementAnimateOnLine(ctx, pen, 'element', animateEle);
      }
      break;
    default:
      if (pen.animateReverse) {
...
function setElementAnimateOnLine(
  ctx: CanvasRenderingContext2D,
  line: Pen,
  type: string,
  element: any,
  ...
  switch (type) {
    case 'image':
      ...
      break;
    case 'icon':
      draw = lineAnimateIconRender(element);
      break;
    case 'pen':
      let targetPens = line.calculative.canvas.find(element);
      line.lineAnimateElementCount = targetPens.length;
      draw = lineAnimatePenRender(targetPens);
      break;
    case 'element':
...
function lineAnimateImageRender(imgs: HTMLImageElement[]) {
  return function (
    ...
    ctx.beginPath();
    ctx.translate(
      state.x + (width / 2) * scale,
      state.y + (height / 2) * scale,
    ); // 平移到中心点
...
function lineAnimatePenRender(tracks: Pen[]) {
  return function (
    ...
    const targetPen = tracks[index % tracks.length];
    if (!targetPen) return;
    const scale = pen.calculative.canvas.store.data.scale;
    const origin = pen.calculative.canvas.store.data.origin;
    ...
    const worldX = state.x - width / 2;
    const worldY = state.y - height / 2;
...
function lineAnimateIconRender(icon: any) {
  return function (
    ...
    const fontFamily = pen.calculative.iconFamily;
    fontSize =
      pen.calculative.iconSize ||
      Math.max(
        pen.lineAnimateElementWidth || 0,
        pen.lineAnimateElementHeight || 0,
      ) ||
      10;
    ctx.font = getFont({
      fontSize,
    ...
    ctx.fillStyle =
      pen.calculative.iconColor ||
      getTextColor(pen, pen.calculative.canvas.parent.store);
...
function renderElementOnLine(
  ...
  const dash = Array.isArray(line.lineAnimateDash)
    ? line.lineAnimateDash
    : line.lineAnimateDash?.split(',').map((i) => Number(i)) || [10, 20];
  const scale = line.calculative.canvas.store.data.scale;
  const len = getLineLength(line) / scale;
  const elesPos = computeLineDashSegments(
    len,
    dash,
    line.lineAnimateDashOffset,
    line.lineAnimateElementCount,
  );
 ...
 */
function computeLineDashSegments(
  lineLength: number,
  dashArray: number[],
  offset = 0,
  count: number,
  ...
  for (const k in frame) {
    if (k === 'duration') {
      ...
      pen.calculative.worldRect.center.x =
        pen.calculative.initRect.center.x + lastVal;
      ...
      pen.calculative.worldRect.width =
        pen.calculative.initRect.width + lastVal;
      pen.calculative.worldRect.ex = pen.calculative.initRect.ex + lastVal;
      pen.calculative.worldRect.center.x =
        pen.calculative.initRect.center.x + lastVal;
      ...
      pen.calculative.worldRect.height =
        pen.calculative.initRect.height + lastVal;
      pen.calculative.worldRect.ey = pen.calculative.initRect.ey + lastVal;
      pen.calculative.worldRect.center.y =
        pen.calculative.initRect.center.y + lastVal;
...
export function setLineAnimate(pen: Pen, now: number) {
  ...
  if (pen.curveAnimate) {
    ...
    if (!pen.animateTimingFunction) {
      ...
      timeParams = Array.isArray(pen.animateTimingFunction)
  ...
  if (!pen.calculative.start) {
    ...
    (pen.curveAnimate && elapsed > pen.duration)
  ) {
    // 播放到尾了
...
export function setElemPosition(pen: Pen, elem: HTMLElement) {
  ...
  if (pen.name === 'iframe') {
    elem.style.opacity = pen.calculative.inView ? '1' : '0'; // 是否隐藏元素
  } else {
    elem.style.display =
...
export function calcInView(pen: Pen, calcChild = false) {
  ...
  if (!isShowChild(pen, store)) {
    ...
    if (
      ...
      pen.calculative.inView = pen.frames.some((obj) =>
  ...
  switch (command.c) {
    ...
    case '_font':
      return {
        ...
        v: {
          value:
            command.v.fontSize * pen.calculative.canvas.store.data.scale +
            'px ' +
            (command.v.fontFamily ||
              pen.calculative.canvas.store.options.fontFamily),
        },
      };
    case '_fillStyle':
      return {
...
function dealWithVisio(command, pen, startX, startY) {
  ...
  switch (command.c) {
    ...
    case 'EllipticalArcTo':
      ...
      const crossProduct =
        (endX - startX) * (ctrlY - startY) -
          (endY - startY) * (ctrlX - startX) >
        0;
    ...
    case 'RelEllipticalArcTo':
      ...
      //
      const crossProduct2 =
        (endX3 - startX) * (ctrlY3 - startY) -
          (endY3 - startY) * (ctrlX3 - startX) >
        0;
